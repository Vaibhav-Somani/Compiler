...
@Test 
public void imageGenBlack() throws Exception {
	devel = false;
	grade = true;
	String prog = "imageGenBlack";
	String input = prog
			+ "\nimage[500,274]g;\n"
			+ "g[[x,y]] = 256 + 65280 + 16711680;"
			+ "g -> SCREEN;\n"
			;
	byte[] bytecode = genCode(input);		
	String[] commandLineArgs = {}; 
	...
	BufferedImage image = RuntimeLog.globalImageLog.get(0);
	ImageSupport.compareImages(imageRef, image);*/
	BufferedImage imageRef = ImageSupport.makeConstantImage(0x000000, 500, 274);
	BufferedImage image = RuntimeLog.globalImageLog.get(0);
	assertTrue(ImageSupport.compareImages(imageRef, image));
	keepFrame();
}
...
public void imageGenWhite() throws Exception {
	...
	BufferedImage image = RuntimeLog.globalImageLog.get(0);
	ImageSupport.compareImages(imageRef, image);*/
	BufferedImage imageRef = ImageSupport.makeConstantImage(0xFFFFFF, 256, 512);
	BufferedImage image = RuntimeLog.globalImageLog.get(0);
	assertTrue(ImageSupport.compareImages(imageRef, image));
	keepFrame();
}
...
public void imageGen2() throws Exception {
	...
	for(int i = 0; i < 512; i++) {
		for (int j = 0; j < 512; j++) {
			int pixelRef = i; 
			int pixel = ImageSupport.getPixel(loggedImage, i,j);
			assertEquals(pixelRef, pixel);
		}
	}
		keepFrame();
...
public void imageGen5() throws Exception {
	...
	for(int i = 0; i < 1024; i++) {
		for (int j = 0; j < 1024; j++) {
			int pixelRef = (int) (Math.hypot(i, j)) * 100; 
			int pixel = ImageSupport.getPixel(loggedImage, i,j);
			assertEquals(pixelRef, pixel);
		}
	}
