/**
 * /**
 * JUunit tests for the Scanner for the class project in COP5556 Programming Language Principles 
 * at the University of Florida, Fall 2017.
 * 
 * This software is solely for the educational benefit of students 
 * enrolled in the course during the Fall 2017 semester.  
 * 
 * This software, and any software derived from it,  may not be shared with others or posted to public web sites,
 * either during the course or afterwards.
 * 
 *  @Beverly A. Sanders, 2017
 */

package cop5556fa17;

import static org.junit.Assert.*;

import org.junit.Rule;
import org.junit.Test;
import org.junit.rules.ExpectedException;

import cop5556fa17.Scanner.Kind;
import cop5556fa17.Scanner.LexicalException;
import cop5556fa17.Scanner.Token;

import static cop5556fa17.Scanner.Kind.*;

public class ScannerTest {

	//set Junit to be able to catch exceptions
	@Rule
	public ExpectedException thrown = ExpectedException.none();

	
	//To make it easy to print objects and turn this output on and off
	static final boolean doPrint = true;
	private void show(Object input) {
		if (doPrint) {
			System.out.println(input.toString());
		}
	}

	/**
	 *Retrieves the next token and checks that it is an EOF token. 
	 *Also checks that this was the last token.
	 *
	 * @param scanner
	 * @return the Token that was retrieved
	 */
	
	Token checkNextIsEOF(Scanner scanner) {
		Scanner.Token token = scanner.nextToken();
		assertEquals(Scanner.Kind.EOF, token.kind);
		assertFalse(scanner.hasTokens());
		return token;
	}


	/**
	 * Retrieves the next token and checks that its kind, position, length, line, and position in line
	 * match the given parameters.
	 * 
	 * @param scanner
	 * @param kind
	 * @param pos
	 * @param length
	 * @param line
	 * @param pos_in_line
	 * @return  the Token that was retrieved
	 */
	Token checkNext(Scanner scanner, Scanner.Kind kind, int pos, int length, int line, int pos_in_line) {
		Token t = scanner.nextToken();
		assertEquals(scanner.new Token(kind, pos, length, line, pos_in_line), t);
		return t;
	}

	/**
	 * Retrieves the next token and checks that its kind and length match the given
	 * parameters.  The position, line, and position in line are ignored.
	 * 
	 * @param scanner
	 * @param kind
	 * @param length
	 * @return  the Token that was retrieved
	 */
	Token check(Scanner scanner, Scanner.Kind kind, int length) {
		Token t = scanner.nextToken();
		assertEquals(kind, t.kind);
		assertEquals(length, t.length);
		return t;
	}

	/**
	 * Simple test case with a (legal) empty program
	 *   
	 * @throws LexicalException
	 */
	@Test
	public void testEmpty() throws LexicalException {
		String input = "";  //The input is the empty string.  This is legal
		show(input);        //Display the input 
		Scanner scanner = new Scanner(input).scan();  //Create a Scanner and initialize it
		show(scanner);   //Display the Scanner
		checkNextIsEOF(scanner);  //Check that the only token is the EOF token.
	}
	
	/**
	 * Test illustrating how to put a new line in the input program and how to
	 * check content of tokens.
	 * 
	 * Because we are using a Java String literal for input, we use \n for the
	 * end of line character. (We should also be able to handle \n, \r, and \r\n
	 * properly.)
	 * 
	 * Note that if we were reading the input from a file, as we will want to do 
	 * later, the end of line character would be inserted by the text editor.
	 * Showing the input will let you check your input is what you think it is.
	 * 
	 * @throws LexicalException
	 */
	@Test
	public void testSemi() throws LexicalException {
		String input = "abc";
		Scanner scanner = new Scanner(input).scan();
		show(input);
		show(scanner);
		//checkNext(scanner, IDENTIFIER, 0, 5, 1, 1);
		checkNext(scanner, IDENTIFIER, 0, 3, 1, 1);
		/*checkNext(scanner, OP_ASSIGN, 1, 1, 1, 2);
		checkNext(scanner, INTEGER_LITERAL, 2, 1, 1, 3);*/
		checkNextIsEOF(scanner);
		/*show(input);
		thrown.expect(LexicalException.class);  //Tell JUnit to expect a LexicalException
		try {
			new Scanner(input).scan();
		} catch (LexicalException e) {  //
			show(e);
			assertEquals(0,e.getPos());
			throw e;
		}*/
	}
	
	@Test
	public void testSemi1() throws LexicalException {
		String input = "12345678";
		Scanner scanner = new Scanner(input).scan();
		show(input);
		show(scanner);
		//checkNext(scanner, IDENTIFIER, 0, 5, 1, 1);
		/*checkNext(scanner, IDENTIFIER, 0, 1, 1, 1);
		checkNext(scanner, OP_ASSIGN, 1, 1, 1, 2);*/
		checkNext(scanner, INTEGER_LITERAL, 0, 8, 1, 1);
		checkNextIsEOF(scanner);
	}
	
	@Test
	public void testSemi2() throws LexicalException {
		String input = "\"a\tb\"";
		//System.out.println(input);
		String x = "Hello";
		Scanner scanner = new Scanner(input).scan();
		show(input);
		show(scanner);
		//checkNext(scanner, IDENTIFIER, 0, 5, 1, 1);
		/*checkNext(scanner, IDENTIFIER, 0, 1, 1, 1);
		checkNext(scanner, OP_ASSIGN, 1, 1, 1, 2);*/
		checkNext(scanner, STRING_LITERAL, 0, 5, 1, 1);
		checkNextIsEOF(scanner);
	}
	
	@Test
	public void testSemi3() throws LexicalException {
		String input = "\"c\\\"d\"";
		//System.out.println(input);
		String x = "Hello";
		Scanner scanner = new Scanner(input).scan();
		show(input);
		show(scanner);
		//checkNext(scanner, IDENTIFIER, 0, 5, 1, 1);
		/*checkNext(scanner, IDENTIFIER, 0, 1, 1, 1);
		checkNext(scanner, OP_ASSIGN, 1, 1, 1, 2);*/
		checkNext(scanner, STRING_LITERAL, 0, 6, 1, 1);
		checkNextIsEOF(scanner);
	}
	
	@Test
	public void testSemi4() throws LexicalException {
		String input = "\"c\\\'d\"123_$abc;:";
		//System.out.println(input);
		String x = "Hello";
		Scanner scanner = new Scanner(input).scan();
		show(input);
		show(scanner);
		//checkNext(scanner, IDENTIFIER, 0, 5, 1, 1);
		checkNext(scanner, STRING_LITERAL, 0, 6, 1, 1);
		checkNext(scanner, INTEGER_LITERAL, 6, 3, 1, 7);
		checkNext(scanner, IDENTIFIER, 9, 5, 1, 10);
		checkNext(scanner, SEMI, 14, 1, 1, 15);
		checkNext(scanner, OP_COLON, 15, 1, 1, 16);
		checkNextIsEOF(scanner);
	}
	
	@Test
	public void testSemi5() throws LexicalException {
		String input = "true>= false\r\n\" \"";
		//System.out.println(input);
		//String x = "Hello";
		Scanner scanner = new Scanner(input).scan();
		show(input);
		show(scanner);
		//checkNext(scanner, IDENTIFIER, 0, 5, 1, 1);
		checkNext(scanner, BOOLEAN_LITERAL, 0, 4, 1, 1);
		checkNext(scanner, OP_GE, 4, 2, 1, 5);
		checkNext(scanner, BOOLEAN_LITERAL, 7, 5, 1, 8);
		checkNext(scanner, STRING_LITERAL, 14, 3, 2, 1);
		checkNextIsEOF(scanner);
	}
	
	@Test
	public void testSemi6() throws LexicalException {
		String input = "x;:->";
		//System.out.println(input);
		String x = "Hello";
		Scanner scanner = new Scanner(input).scan();
		show(input);
		show(scanner);
		//checkNext(scanner, IDENTIFIER, 0, 5, 1, 1);
		checkNext(scanner, KW_x, 0, 1, 1, 1);
		checkNext(scanner, SEMI, 1, 1, 1, 2);
		checkNext(scanner, OP_COLON, 2, 1, 1, 3);
		checkNext(scanner, OP_RARROW, 3, 2, 1, 4);
		checkNextIsEOF(scanner);
	}
	/**
	 * This example shows how to test that your scanner is behaving when the
	 * input is illegal.  In this case, we are giving it a String literal
	 * that is missing the closing ".  
	 * 
	 * Note that the outer pair of quotation marks delineate the String literal
	 * in this test program that provides the input to our Scanner.  The quotation
	 * mark that is actually included in the input must be escaped, \".
	 * 
	 * The example shows catching the exception that is thrown by the scanner,
	 * looking at it, and checking its contents before rethrowing it.  If caught
	 * but not rethrown, then JUnit won't get the exception and the test will fail.  
	 * 
	 * The test will work without putting the try-catch block around 
	 * new Scanner(input).scan(); but then you won't be able to check 
	 * or display the thrown exception.
	 * 
	 * @throws LexicalException
	 */
	@Test
	public void failUnclosedStringLiteral() throws LexicalException {
		String input = "\" greetings  ";
		show(input);
		thrown.expect(LexicalException.class);  //Tell JUnit to expect a LexicalException
		try {
			new Scanner(input).scan();
		} catch (LexicalException e) {  //
			show(e);
			assertEquals(13,e.getPos());
			throw e;
		}
	}
	
	@Test
	public void invalidInput() throws LexicalException {
		String input = "   .";
		//System.out.println(input);
		show(input);
		thrown.expect(LexicalException.class);  //Tell JUnit to expect a LexicalException
		try {
			new Scanner(input).scan();
		} catch (LexicalException e) {  //
			show(e);
			assertEquals(3,e.getPos());
			throw e;
		}
	}
	
	@Test
	public void invalidString1() throws LexicalException {
		String input = "\"abc\"\"cde ";
		show(input);
		thrown.expect(LexicalException.class);  //Tell JUnit to expect a LexicalException
		try {
			new Scanner(input).scan();
		} catch (LexicalException e) {  //
			show(e);
			assertEquals(10,e.getPos());
			throw e;
		}
	}
	
	@Test
	public void invalidInteger() throws LexicalException {
		String input = "12345678910112";
		show(input);
		thrown.expect(LexicalException.class);  //Tell JUnit to expect a LexicalException
		try {
			new Scanner(input).scan();
		} catch (LexicalException e) {  //
			show(e);
			assertEquals(0,e.getPos());
			throw e;
		}
	}
	
	@Test
	public void invalidString2() throws LexicalException {
		String input = "\"abc\nbcd\" ";
		show(input);
		thrown.expect(LexicalException.class);  //Tell JUnit to expect a LexicalException
		try {
			new Scanner(input).scan();
		} catch (LexicalException e) {  //
			show(e);
			assertEquals(4,e.getPos());
			throw e;
		}
	}
	
	@Test
	public void invalidString3() throws LexicalException {
		String input = ";\"abc\\ucd\" ";
		show(input);
		thrown.expect(LexicalException.class);  //Tell JUnit to expect a LexicalException
		try {
			new Scanner(input).scan();
		} catch (LexicalException e) {  //
			show(e);
			assertEquals(6,e.getPos());
			throw e;
		}
	}
	
	@Test
	public void testSemi7() throws LexicalException {
		String input = "\"abc\\\\def\"\n\r0";
		Scanner scanner = new Scanner(input).scan();
		show(input);
		show(scanner);
		//checkNext(scanner, IDENTIFIER, 0, 5, 1, 1);
		/*checkNext(scanner, IDENTIFIER, 0, 1, 1, 1);
		checkNext(scanner, OP_ASSIGN, 1, 1, 1, 2);*/
		checkNext(scanner, STRING_LITERAL, 0, 10, 1, 1);
		checkNext(scanner, INTEGER_LITERAL, 12, 1, 3, 1);
		checkNextIsEOF(scanner);
	}
	
	@Test
	public void invalidString4() throws LexicalException {
		String input = "\"abc\"\"";
		show(input);
		thrown.expect(LexicalException.class);  //Tell JUnit to expect a LexicalException
		try {
			new Scanner(input).scan();
		} catch (LexicalException e) {  //
			show(e);
			assertEquals(6,e.getPos());
			throw e;
		}
	}
	
	@Test
	public void testSemi8() throws LexicalException {
		String input = "\"abc\\\\def\"\n\r0";
		Scanner scanner = new Scanner(input).scan();
		show(input);
		show(scanner);
		//checkNext(scanner, IDENTIFIER, 0, 5, 1, 1);
		/*checkNext(scanner, IDENTIFIER, 0, 1, 1, 1);
		checkNext(scanner, OP_ASSIGN, 1, 1, 1, 2);*/
		checkNext(scanner, STRING_LITERAL, 0, 10, 1, 1);
		checkNext(scanner, INTEGER_LITERAL, 12, 1, 3, 1);
		checkNextIsEOF(scanner);
	}
	
	@Test
	public void testSemi9() throws LexicalException {
		String input = "sin(x) + cos[y] == 1";
		Scanner scanner = new Scanner(input).scan();
		show(input);
		show(scanner);
		//checkNext(scanner, IDENTIFIER, 0, 5, 1, 1);
		/*checkNext(scanner, IDENTIFIER, 0, 1, 1, 1);
		checkNext(scanner, OP_ASSIGN, 1, 1, 1, 2);*/
		checkNext(scanner, KW_sin, 0, 3, 1, 1);
		checkNext(scanner, LPAREN, 3, 1, 1, 4);
		checkNext(scanner, KW_x, 4, 1, 1, 5);
		checkNext(scanner, RPAREN, 5, 1, 1, 6);
		checkNext(scanner, OP_PLUS, 7, 1, 1, 8);
		checkNext(scanner, KW_cos, 9, 3, 1, 10);
		checkNext(scanner, LSQUARE, 12, 1, 1, 13);
		checkNext(scanner, KW_y, 13, 1, 1, 14);
		checkNext(scanner, RSQUARE, 14, 1, 1, 15);
		checkNext(scanner, OP_EQ, 16, 2, 1, 17);
		checkNext(scanner, INTEGER_LITERAL, 19, 1, 1, 20);
		checkNextIsEOF(scanner);
	}
	
	@Test
	public void testSemi10() throws LexicalException {
		String input = "\"a\tb\"";
		Scanner scanner = new Scanner(input).scan();
		show(input);
		show(scanner);
		//checkNext(scanner, IDENTIFIER, 0, 5, 1, 1);
		/*checkNext(scanner, IDENTIFIER, 0, 1, 1, 1);
		checkNext(scanner, OP_ASSIGN, 1, 1, 1, 2);*/
		checkNext(scanner, STRING_LITERAL, 0, 5, 1, 1);
		//checkNext(scanner, INTEGER_LITERAL, 12, 1, 3, 1);
		checkNextIsEOF(scanner);
	}
	
	@Test
	public void testSemi11() throws LexicalException {
		String input = "polar_r**url&file?cart_x@image";
		Scanner scanner = new Scanner(input).scan();
		show(input);
		show(scanner);
		//checkNext(scanner, IDENTIFIER, 0, 5, 1, 1);
		/*checkNext(scanner, IDENTIFIER, 0, 1, 1, 1);
		checkNext(scanner, OP_ASSIGN, 1, 1, 1, 2);*/
		checkNext(scanner, KW_polar_r, 0, 7, 1, 1);
		checkNext(scanner, OP_POWER, 7, 2, 1, 8);
		checkNext(scanner, KW_url, 9, 3, 1, 10);
		checkNext(scanner, OP_AND, 12, 1, 1, 13);
		checkNext(scanner, KW_file, 13, 4, 1, 14);
		checkNext(scanner, OP_Q, 17, 1, 1, 18);
		checkNext(scanner, KW_cart_x, 18, 6, 1, 19);
		checkNext(scanner, OP_AT, 24, 1, 1, 25);
		checkNext(scanner, KW_image, 25, 5, 1, 26);
		checkNextIsEOF(scanner);
	}
	
	@Test
	public void testSemi12() throws LexicalException {
		String input = "\"he\b\"";
		System.out.println(input);
		Scanner scanner = new Scanner(input).scan();
		show(input);
		show(scanner);
		//checkNext(scanner, IDENTIFIER, 0, 5, 1, 1);
		/*checkNext(scanner, IDENTIFIER, 0, 1, 1, 1);
		checkNext(scanner, OP_ASSIGN, 1, 1, 1, 2);*/
		//checkNext(scanner, IDENTIFIER, 0, 1, 1, 1);
		checkNext(scanner, STRING_LITERAL, 0, 5, 1, 1);
		checkNextIsEOF(scanner);
	}
	
	@Test
	public void testSemi13() throws LexicalException {
		String input = "\"he\b\"";
		System.out.println(input);
		Scanner scanner = new Scanner(input).scan();
		show(input);
		show(scanner);
		//checkNext(scanner, IDENTIFIER, 0, 5, 1, 1);
		/*checkNext(scanner, IDENTIFIER, 0, 1, 1, 1);
		checkNext(scanner, OP_ASSIGN, 1, 1, 1, 2);*/
		//checkNext(scanner, IDENTIFIER, 0, 1, 1, 1);
		checkNext(scanner, STRING_LITERAL, 0, 5, 1, 1);
		checkNextIsEOF(scanner);
	}
	
	@Test
	public void testSemi14() throws LexicalException {
		String input = "\"he\"";
		//System.out.println(input);
		Scanner scanner = new Scanner(input).scan();
		show(input);
		show(scanner);
		//checkNext(scanner, IDENTIFIER, 0, 5, 1, 1);
		/*checkNext(scanner, IDENTIFIER, 0, 1, 1, 1);
		checkNext(scanner, OP_ASSIGN, 1, 1, 1, 2);*/
		//checkNext(scanner, IDENTIFIER, 0, 1, 1, 1);
		checkNext(scanner, STRING_LITERAL, 0, 4, 1, 1);
		checkNextIsEOF(scanner);
	}
	
	@Test
	public void testSemi15() throws LexicalException {
		String input = "\"abcde\"";
		//System.out.println(input);
		Scanner scanner = new Scanner(input).scan();
		show(input);
		show(scanner);
		//checkNext(scanner, IDENTIFIER, 0, 5, 1, 1);
		/*checkNext(scanner, IDENTIFIER, 0, 1, 1, 1);
		checkNext(scanner, OP_ASSIGN, 1, 1, 1, 2);*/
		//checkNext(scanner, IDENTIFIER, 0, 1, 1, 1);
		checkNext(scanner, STRING_LITERAL, 0, 7, 1, 1);
		checkNextIsEOF(scanner);
	}
	
	@Test
	public void testSemi16() throws LexicalException {
		String input = "00012_%";
		//System.out.println(input);
		Scanner scanner = new Scanner(input).scan();
		show(input);
		show(scanner);
		//checkNext(scanner, IDENTIFIER, 0, 5, 1, 1);
		/*checkNext(scanner, IDENTIFIER, 0, 1, 1, 1);
		checkNext(scanner, OP_ASSIGN, 1, 1, 1, 2);*/
		//checkNext(scanner, IDENTIFIER, 0, 1, 1, 1);
		checkNext(scanner, INTEGER_LITERAL, 0, 1, 1, 1);
		checkNext(scanner, INTEGER_LITERAL, 1, 1, 1, 2);
		checkNext(scanner, INTEGER_LITERAL, 2, 1, 1, 3);
		checkNext(scanner, INTEGER_LITERAL, 3, 2, 1, 4);
		checkNext(scanner, IDENTIFIER, 5, 1, 1, 6);
		checkNext(scanner, OP_MOD, 6, 1, 1, 7);
		checkNextIsEOF(scanner);
	}
	
	@Test
	public void testSemi17() throws LexicalException {
		String input = "\"ab\\\"cd\"";
		//System.out.println(input);
		Scanner scanner = new Scanner(input).scan();
		show(input);
		show(scanner);
		//checkNext(scanner, IDENTIFIER, 0, 5, 1, 1);
		/*checkNext(scanner, IDENTIFIER, 0, 1, 1, 1);
		checkNext(scanner, OP_ASSIGN, 1, 1, 1, 2);*/
		//checkNext(scanner, IDENTIFIER, 0, 1, 1, 1);
		checkNext(scanner, STRING_LITERAL, 0, 8, 1, 1);
		checkNextIsEOF(scanner);
	}
	
	@Test
	public void comment1() throws LexicalException {
		String input = "abc//avc\r\n_@";
		//System.out.println(input);
		Scanner scanner = new Scanner(input).scan();
		show(input);
		show(scanner);
		//checkNext(scanner, IDENTIFIER, 0, 5, 1, 1);
		/*checkNext(scanner, IDENTIFIER, 0, 1, 1, 1);
		checkNext(scanner, OP_ASSIGN, 1, 1, 1, 2);*/
		//checkNext(scanner, IDENTIFIER, 0, 1, 1, 1);
		checkNext(scanner, IDENTIFIER, 0, 3, 1, 1);
		checkNext(scanner, IDENTIFIER, 10, 1, 2, 1);
		checkNext(scanner, Kind.OP_AT, 11, 1, 2, 2);
		checkNextIsEOF(scanner);
	}
	
	@Test
	public void testsemi18() throws LexicalException {
		String input = "\"abc\\bc\"";
		//System.out.println(input);
		Scanner scanner = new Scanner(input).scan();
		show(input);
		show(scanner);
		//checkNext(scanner, IDENTIFIER, 0, 5, 1, 1);
		/*checkNext(scanner, IDENTIFIER, 0, 1, 1, 1);
		checkNext(scanner, OP_ASSIGN, 1, 1, 1, 2);*/
		//checkNext(scanner, IDENTIFIER, 0, 1, 1, 1);
		checkNext(scanner, STRING_LITERAL, 0, 8, 1, 1);
		checkNextIsEOF(scanner);
	}
	
	@Test
	public void testsemi19() throws LexicalException {
		String input = "ab\r\ndc";
		//System.out.println(input);
		Scanner scanner = new Scanner(input).scan();
		show(input);
		show(scanner);
		//checkNext(scanner, IDENTIFIER, 0, 5, 1, 1);
		/*checkNext(scanner, IDENTIFIER, 0, 1, 1, 1);
		checkNext(scanner, OP_ASSIGN, 1, 1, 1, 2);*/
		//checkNext(scanner, IDENTIFIER, 0, 1, 1, 1);
		checkNext(scanner, IDENTIFIER, 0, 2, 1, 1);
		checkNext(scanner, IDENTIFIER, 4, 2, 2, 1);
		checkNextIsEOF(scanner);
	}
	
	@Test
	public void comment2() throws LexicalException {
		String input = "ab//cv\rdnv";
		//System.out.println(input);
		Scanner scanner = new Scanner(input).scan();
		show(input);
		show(scanner);
		//checkNext(scanner, IDENTIFIER, 0, 5, 1, 1);
		/*checkNext(scanner, IDENTIFIER, 0, 1, 1, 1);
		checkNext(scanner, OP_ASSIGN, 1, 1, 1, 2);*/
		//checkNext(scanner, IDENTIFIER, 0, 1, 1, 1);
		checkNext(scanner, IDENTIFIER, 0, 2, 1, 1);
		checkNext(scanner, IDENTIFIER, 7, 3, 2, 1);
		//checkNext(scanner, Kind.OP_AT, 11, 1, 2, 2);
		checkNextIsEOF(scanner);
	}
	
	@Test
	public void comment3() throws LexicalException {
		String input = "ab//cvdnv";
		//System.out.println(input);
		Scanner scanner = new Scanner(input).scan();
		show(input);
		show(scanner);
		//checkNext(scanner, IDENTIFIER, 0, 5, 1, 1);
		/*checkNext(scanner, IDENTIFIER, 0, 1, 1, 1);
		checkNext(scanner, OP_ASSIGN, 1, 1, 1, 2);*/
		//checkNext(scanner, IDENTIFIER, 0, 1, 1, 1);
		checkNext(scanner, IDENTIFIER, 0, 2, 1, 1);
		//checkNext(scanner, IDENTIFIER, 7, 3, 2, 1);
		//checkNext(scanner, Kind.OP_AT, 11, 1, 2, 2);
		checkNextIsEOF(scanner);
	}
	
	@Test
	public void testsem20() throws LexicalException {
		String input = "\"\\'\"";
		//System.out.println(input);
		Scanner scanner = new Scanner(input).scan();
		show(input);
		show(scanner);
		//checkNext(scanner, IDENTIFIER, 0, 5, 1, 1);
		/*checkNext(scanner, IDENTIFIER, 0, 1, 1, 1);
		checkNext(scanner, OP_ASSIGN, 1, 1, 1, 2);*/
		//checkNext(scanner, IDENTIFIER, 0, 1, 1, 1);
		checkNext(scanner, STRING_LITERAL, 0, 4, 1, 1);
		//checkNext(scanner, IDENTIFIER, 7, 3, 2, 1);
		//checkNext(scanner, Kind.OP_AT, 11, 1, 2, 2);
		checkNextIsEOF(scanner);
	}
	
	@Test
	public void testsem21() throws LexicalException {
		String input = "\"\"\"";
		show(input);
		thrown.expect(LexicalException.class);  //Tell JUnit to expect a LexicalException
		try {
			new Scanner(input).scan();
		} catch (LexicalException e) {  
			show(e);
			assertEquals(3,e.getPos());
			throw e;
		}
	}
	
	@Test
	public void invalidString5() throws LexicalException {
		String input = "\"abc\"\" ";
		show(input);
		thrown.expect(LexicalException.class);  //Tell JUnit to expect a LexicalException
		try {
			new Scanner(input).scan();
		} catch (LexicalException e) {  //
			show(e);
			assertEquals(7,e.getPos());
			throw e;
		}
	}
	
	@Test
	public void invalidInput1() throws LexicalException {
		String input = " ^";
		show(input);
		thrown.expect(LexicalException.class);  //Tell JUnit to expect a LexicalException
		try {
			new Scanner(input).scan();
		} catch (LexicalException e) {  //
			show(e);
			assertEquals(1,e.getPos());
			throw e;
		}
	}
	
	@Test
	public void testsem23() throws LexicalException {
		String input = "01 12 3\n23 4567890";
		//System.out.println(input);
		Scanner scanner = new Scanner(input).scan();
		show(input);
		show(scanner);
		checkNext(scanner, INTEGER_LITERAL, 0, 1, 1, 1);
		checkNext(scanner, INTEGER_LITERAL, 1, 1, 1, 2);
		checkNext(scanner, INTEGER_LITERAL, 3, 2, 1, 4);
		checkNext(scanner, INTEGER_LITERAL, 6, 1, 1, 7);
		checkNext(scanner, INTEGER_LITERAL, 8, 2, 2, 1);
		checkNext(scanner, INTEGER_LITERAL, 11, 7, 2, 4);
		//checkNext(scanner, Kind.OP_AT, 11, 1, 2, 2);
		checkNextIsEOF(scanner);
	}
}
